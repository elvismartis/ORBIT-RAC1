...
Small GTPases show significant role in regulating many cellular processes owing to their switching mechanism, wherein GTP-bound state is called the \textit{Active state} and the GDP-bound is called the \textit{Inactive state}. As shown earlier, nucleotides bind tightly to the GTPases, their dissociation is assisted by Guanine Exchange Factors (GEFs).  There are several exchanges factors available for this purpose, and specific GEFs active specific GTPases. In case of RAC1, PREX1, TIAM1, VAV1 and DOCK2 are specific GEFs implicated in a wide variety of diseases. Structurally, PREX1, TIAM1 and VAV1 are DH/PH domain containing proteins, while DOCK2 contains DHR domain. Irrespective of their structural diversity, all GEFs bind to the switch I and II regions and assist in the nucleotide exchange, i.e. dissociation of GDP and association of GTP. The nucleotide binding site is well conserved among the GTPase family, and therefore, making it challenging to target for finding small molecule binders. This problem is worsened by the molar concentrations of GTPs with picomolar levels of binding affinity. Therefore, we embarked on understanding the complete dynamic landscape of RAC1 protein bound to either nucleotides (GDP or GTP) or GEFs (PREX1, TIAM1, VAV1 or DOCK2). We expect to locate alternate binding pockets, that may improve the druggability of RAC1 protein. Furthermore, we measure the pocket volume changes of the newly identified pockets and try to identify if there exists any correlation in their volume changes amongst other pockets or with the nucleotide binding site. Since, the different GEFs can be implicated in more than disease states, we attempt to establish if certain pockets are more open or closed depending on the RAC1 binding partner. This could mean that, different pocket(s) can be targeted at different  times to selectively modulate or prevent a specific GEF from activating RAC1. 
...
\begin{figure}[ht]
    ...
    \caption{Binding Pockets on RAC1 surface. \textbf{A)}  \textbf{B)}  \textbf{C)}  \textbf{D)}  \textbf{E)}  \textbf{F)} The normalised distribution of the root-mean-squared deviation of the backbone atoms of the Switch I region. \textbf{F)}} 
    \label{fig:RAC1pockets}
\end{figure}
