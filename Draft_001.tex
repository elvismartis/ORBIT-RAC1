...
The nucleotide binding site is well conserved among the GTPase family, and therefore, posing significant challenges to target it. The molar concentrations of GTPs and picomolar binding affinity makes it more difficult for the competing small molecules. The switching mechanism offers a conformational landscape that can be studied and new binding sites can be located. Furthermore, the associate of the exchange factors shows the existence of alternative binding sites that might transient open to allow binding of other proteins. Therefore, we embarked to understand the complete dynamical landscape of Rac1 protein bound to either nucleotides (GDP or GTP) or GEFs (PREX1, TIAM1, VAV1 or DOCK2). We expect to locate alternate binding pockets that may improve the druggability of Rac1 protein. We also attempt to understand if different binding parters show differences in the pockets we identify which can be exploited to selectively target Rac1 implicated in different signalling pathways. We hypothesise that the selectivity can be measured as the function of the pocket volume changes. we also attempt to establish if there exists any correlation in the volume changes amongst other pockets or with the nucleotide binding site. The latter can be believed to influence the nucleotide traffic and eventually control the switching mechanism of Rac1. 
 \newpage
\subsection{GEF assistence opening of transient pockets}
...
We employed the POVME2 tool to identify and measure the pocket volume changes over the course of the MD trajectory. To locate new transient pockets, we simulated Rac1 under different conditions (See \textbf{Table \ref{tbl: Detailed system studies}}). Prior to identifying the binding pockets, all binding partners including nucleotides were deleted from the trajectory. This allowed us to focus entirely on the conformational dynamics of Rac1 for new pocket identification. The pockets were identified using the starting conformation of Rac1 X-ray structure, and the pocket volume changes were monitored throughout the course of the trajectory. 
We measured pocket volume changes in the nucleotide binding site in response to various Rac1 binding partners. The presence of nucleotides (GDP or GTP) in their site restricts the volume fluctuations under 3000\r{A}$^3$ (\textbf{Figure \ref{fig:nucleotidepockets}B}). This is the attributed to tight binding of nucleotides, further stabilised with $Mg^{2+}$ that interacts with the switch regions.  This severely reduces their conformational flexibility the switch regions (\textbf{Figure \ref{fig:MD}D and \ref{fig:MD}E}). The switch region conformation is also found to be reduced by GEF binding (\textbf{Figure \ref{fig:MD}D and \ref{fig:MD}E}) suggesting their role to control the nucleotide traffic by modulating the switching regions.  We tested this hypothesis by simulating a nucleotide-free and GEF-free form of Rac1 (delGDP and delGTP in \textbf{Figure \ref{fig:nucleotidepockets}B}) and found that the mean pocket volume and fluctuations showed a greater tendency to increase. 
The pocket volume fluctuations in the nucleotide binding site was higher in presence of GEFs than in presence of GTP or GDP. The increase in the magnitude was not the same for all GEFs used in this study. For example, PREX1 and DOCK2, showed volume changes with similar magnitude as seen in Rac1 devoid of nucleotides (delGDP and delGTP in \textbf{Figure \ref{fig:nucleotidepockets}B}). Interestingly, TIAM1 and VAV1 showed significantly enlarged nucleotide pocket. These volume fluctuations ranged from 2000\r{A}$^3$ to 4000\r{A}$^3$. We hypothesis that, such difference in pocket volume changes in presence of different GEF suggests that X-ray structure has perhaps captured the Rac1-GEF complexes either just after GDP dissociation (PREX1 and DOCK2) or in a state that was pre-organised for GTP:$Mg^{2+}$ association. 

 \newpage
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\textwidth]{Fig2.jpg}
    \caption{\textbf{A)} Shows the surface model of Rac1 protein with coloured spears illustrating the binding site identified with POVME2. The helical insert is highlighted using the yellow colour. The green region indicates the switch II and orange region indicates the switch I region. The magenta region slightly visible through the red spears indicates the P-loop. The red spears occupy the nucleotide binding site that is lined with helical insert, switch regions and the P-loop forms the phosphate binding region. Blue spears occupy the site P1 and the cyan spears occupy the site P2. Site P3 and P4 are visible by rotating the Rac1 by 90\textdegree. The salmon pink spears occupy site P3 while the olive green spears occupy site P5.  \textbf{B)} The violin plots illustrate the distribution of the pocket volumes for each of the nucletide binding site. The changes in the pocket volumes are monitored when Rac1 is complexed with either GEFs, GDP, GTP or in the \textit{apo}-state.} 
    \label{fig:nucleotidepockets}
\end{figure}

In addition to the nucleotide binding site (\textbf{Figure \ref{fig:nucleotidepockets}A}), we identified four pockets on the Rac1 surface that we believe are potential new binding sites. Site P1  lies are the inter-junction between switch I and II, while site P2 lies slightly below the Site P1. These sites form the interface region where GEFs and GAPs are known to dock to perform their functions (\textbf{Figure \ref{fig:nucleotidepockets}A}).  Site P2 in Rac1 is similar to the one in K-RAS and H-RAS \cite{sun2012discovery, maurer2012small} that is occupied by the covalent inhibitors in G12C RAS variants.  This site is present at a crucial location that can be used to modulate the dynamics of the switch regions. Thereby modulating the binding of upstream of GEFs (\textbf{Figure \ref{fig:otherpockets}B}) can be believed to be GEF specific owing to their different pocket volumes and therefore, accessible to small molecules of different sizes. The is also a possibility to occlude the nucleotide binding site by blocking the switch region conformations in a position such that it is unfavourable for GTP binding. thereby prolonging the nucleotide free state and prevent the downstream signalling. Site P2 located below Site P1, and also found to show different volume changes, rather increased volume, in presence of different GEF (\textbf{Figure \ref{fig:otherpockets}C}). This could serve as a pocket to extend Site P1 binders for more GEF specific control. The targetting of site P1 and/or site P2 will either prevent GEF binding to Rac1, and thereby, prevent activation of GEF-dependent downstream signalling. 

Away from GEF binding site are located site P3 and P4 (\textbf{Figure \ref{fig:nucleotidepockets}A}). Site P3 is located at the starting point of switch I. Targetting this site could modulate the switch I dynamics. This could in return have on an influence of the traffic to the nucleotide binding site as well as GEF binding. Site P4 is the smallest pocket in terms of the volume and the only site with no direct connection to the switch region. 
...
\begin{figure}[ht]
    ...
    \caption{Four binding site identified using POVME2. \textbf{A)} Shows the surface model of Rac1 protein with coloured spears illustrating the binding site identified. The following violin plots illustrate the distribution of the pocket volumes for each of these binding sites on the Rac1 surface monitored when Rac1 is complexed with either GEFs, GDP, GTP or in the \textit{apo}-state. \textbf{B)} Site P1. It is same site of the KRAS-G12C covalent inhibitors binds.   \textbf{C)} Site P2 is found at the bottom of switch I and II loops. \textbf{D)} Site P3 is found at the bottom of the switch I region.  \textbf{E)} Site P4 is only pocket that is not located close to switch I or II regions. The dotted line indicates the minimum volume for druggability of a pocket\cite{perot2010druggable, nayal2006nature, an2005pocketome}.} 
