...
\subsection{Identification and measurement of alternate pockets on Rac1} 
    POVME2 \cite{durrant2011povme, durrant2014povme} was employed for identifying various pockets on the Rac1. To be able to incorporate various experimental conformations of Rac1, we employed Rac1 with different binding partners (see \textbf{Table \ref{tbl:pdbs}}).  To identify the pockets, the pocketID module on POVME2 was used on the aforementioned Rac1 structures. The parameters used for pocket identification are shown in \textbf{Table \ref{tbl:pocketParameters}}. Following the pocket identification, we monitored the pocket volume changes of these pockets throughout MD trajectories. It has been observed that the pocket might be a good candidate for druggability if its volume range was found to lie between 610\r{A}$^3$ to 930\r{A}$^3$\cite{perot2010druggable, nayal2006nature, an2005pocketome}. 
  \begin{table}
  \caption{Parameters used for pocket identification}
  \label{tbl:pocketParameters}
  \begin{tabular}{lc}
    \hline
...
Lastly, we attempted to find out if there existed any correlated pocket volume fluctuations among all the pockets. For this, we used Pearson's correlation cofficient. We observed there is marginal or neglible correlations in the pocket volume fluctions (\textbf{Figures \ref{SI-fig:SI_CORRNUC}} and \textbf{\ref{SI-fig:SI_CORRGEF}}). However, in the deleting GDP from the nucleotide pocket yield the delGDP state shows a strong positive correlation between Site P2 and Site P3. We could not ascertain any noticeable atomic motions that explains this correlation in this isolated case. In general, there exists negligible correlated pocket volume fluctuations. 
...
\begin{figure}[hbt!]
    ...
    \caption{Pocket volume measurements following cosolvent MD simulations for \textbf{(A)} Site P1, \textbf{(B)} site P2, \textbf{(C)} site P3, and \textbf{(D)} site P4. The numerical values on the violin plots indicate the minimum and maximum pocket volumes measured in the MD trajectories.}
    \label{fig:cosol}
\end{figure}
...
\subsection{Switch region dynamics explains formation of transient pockets}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\textwidth]{PCA.jpg}
    \caption{Essential Dynamics of Rac1 protein to explain the opening and closing of the transient pockets induced by the cosolvent probes. Magenta colour indicates the P-loop region. Orange colour indicates switch I region. Green colour indicates switch II region. Yellow colour indicates helical insert. IPA is isopropanol. \textbf{(A)} Projection of amplitude motions from PC1 on Rac1 structure. \textbf{(B)} Identification of Rac1 conformational clusters in various water and cosolvent systems based on essential dynamics using PC1 and PC2.} 
    \label{fig:PCA}
\end{figure}
