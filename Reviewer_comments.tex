...
\textbf{Author Response}: Indeed. We do agree with this concern that there is a lack of direct casual link. We have rephrased the discussion on this part to also indicate to the readers that the microsecond time scale could be a limitation that no spontaneous pocket openings were sampled in the systems without cosolven probes.

The threshold value of 600 $\r{A}^3$ is not entirely arbitary. The following work \textcolor{blue}{1. 10.1016/j.drudis.2010.05.015. 2.  10.1074/mcp.M400159-MCP200 and 3. 10.1002/prot.20897Digital}, do suggest values close to the threshold used in this work. However, we also accept that to best of out knowledge we did not find published reports that report vastly different pocket volumes to classfify them as "druggable" or "undruggable"
...
\textbf{Author Response}: We totally agree with reviewer that entropic contribution are important component of the free energy of binding. In this work, out intension was not to compute the free energy of binding between RAC1 and GEFs. We employed the MMGBSA methods to docompose the binding affinity value to identify residues on the RAC1 surface that show direct interactions with the residues on the GEF surface. While in the GBSA method is enthalpy component is amenable to residuewise decomposition, we are skeptical about the amenability of residue wise decomposition of the entropy component. Therefore, we did not compute the entropy component altogether. We have modified the methods section to indicate the same. 
...
\begin{enumerate}
    ...
    \textbf{Author Response}: We are grateful to the reviewer for this suggestion, we have replaced "cosolvent" with "cosolvent molecular probes" in the enter text. 

    In order understand the proximity of the cosolvent molecular probes to the transient sites, we computed radial distribution function of the probes with the center of mass of the residues that form the transient site. The figure is added in the supplementary figure and appropriate dicussion in the main text. 
    ...
    We did observe correlation in the pocket volumes on 3 occassions, which according to the reviewers are moderate correlation, and we agree with the reviewer. Since in large majority of cases there was no such correlation observed, we decided no to probe further to understand time-dependent correlation. 
    As a matter of fact we did try to understand  the correlation of the volumes of pocket from P1 to P4 with the volume of the nucleotide binding site (see supplementary figure S1 and S2). However,there was no correlation whatsoever.
...
\textbf{Author Response}: We are grateful to the reviewer to point the drawbacks of the PCA in this work. Upon in depth analysis, we oberved that PCA alone is unable to establish a direct causal link between switch region dynamics and pocket opening. We have modified the discussion address the concerns of the reviewer. We have also modified the PCA-based clustering and identified the data points based on the replicate trajectories. 
We followed the recommendations by David and Jacob on PCA analysis to extract essential dynamics of proteins (See: Methods Mol Biol. 2014 ; 1084: 193–226. $10.1007/978-1-62703-658-0\_11$.), 20 eigenvalues are more than sufficient. Also, in this case, the proporation of variance of Eigenvalues beyond 7 sharply falls below 5\% and decays close to 1\% for those above 10. It suggests that higher ranked eigenvalues contribute far less to the over variance than do the first few eigenvalues.
